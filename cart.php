      ...
      <div class="row">
        ...
        while($row=mysqli_fetch_array($result)){
            ...
            ?>
                  <tr>
                      ...
                      <td><input type="number" min="1" name="qty" class="form-input w-50"></td>
              ...
              <?php
                ...
                if(isset($_POST['update_cart'])){
                    ...
                    if($quantity>$quantities){
                      $update_cart="update `cart_details` set quafntity=$quantities where product_id='$update_id'";
                      $result_products_quantity=mysqli_query($con,$update_cart);
                     // $total_price=(int)$total_price*(int)$quantities; 
                
                    $newquantity= $quantity-$quantities;
                    $update_product="update `products` set quantity=$newquantity where product_id='$update_id'";
                    $result_products_quantity2=mysqli_query($con,$update_product);
                    }
                    else{
                      echo "<script>alert('Sorry, This product is currently unavailable, Please come back soon........')</script>";
        ...
        if($result_count>0){
            echo "<h4 class='px-3'>Subtotal: <strong class='text-info'> $total_price /- </strong></h4>
             ...
             <button class='bg-secondary p-3 py-2 border-0 text-light'> <a href='checkout.php' name='checkout'><a href='users_area/checkout.php' class='text-light text-decoration-none'  name='checkout'>Checkout</button></a>";
        }else{
